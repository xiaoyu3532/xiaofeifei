        ...
        if ($result['result_code'] === 'SUCCESS' && $result['mch_id'] === \WxPayConfig::MCHID) {
            ...
            if ($sign === $result['sign']) {
                ...
                try {
                    ...
                    file_put_contents('dadingdan.log', print_r($order_num_data,true).PHP_EOL);
                    if ($order_num_data) {
                        //获取小订单
                        $table1 = 'order';
                        ...
                        file_put_contents('xiaodingdan.log', print_r($order_data,true).PHP_EOL);
                        if (!$order_data) {
                            Db::rollback();
                        }
                        ...
                        foreach ($order_data as $k => $v) {
                            ...
                            file_put_contents('gaixiaodingdan.log', print_r($order_update,true).PHP_EOL);
                            if (!$order_update) {
                                Db::rollback();
                            }
                            file_put_contents('kechengleixing.log', print_r($v['cou_status'],true).PHP_EOL);
                            //机构加销量
                            if ($v['cou_status'] == 1 || $v['cou_status'] == 2 || $v['cou_status'] == 4) {
                                ...
                                file_put_contents('jiaxiaoliang.log', print_r($enroll_num_inc,true).PHP_EOL);
                                if (!$enroll_num_inc) {
                                    Db::rollback();
                                }
                            }
                            ...
                            file_put_contents('bangdingbiaoshihuoyoue.log', print_r($user_member,true).PHP_EOL);
                            if (!$user_member) {
                                if (isset($v['community_id']) && !empty($v['community_id'])) {
                                    $community_id = $v['community_id'];
                                } else {
                                    $community_id = 0;
                                }
                                if (isset($v['syntheticalcn_id']) && !empty($v['syntheticalcn_id'])) {
                                    $syntheticalcn_id = $v['syntheticalcn_id'];
                                } else {
                                    $syntheticalcn_id = 0;
                                }
                                $user_member_data = [
                                ...
                                file_put_contents('bandingyonghu.log', print_r($user_member_belong,true).PHP_EOL);
                            ...
                            file_put_contents('shifuoyouegouwuche.log', print_r($cat_data,true).PHP_EOL);
                            //购物车
                            if ($cat_data) {
...
//                        dump($del_cat);
                            ...
                            file_put_contents('tianjialiushui.log', print_r($account_info,true).PHP_EOL);
                            if (!$account_info) {
                                Db::rollback();
                            }
                        }
                        ...
                        file_put_contents('gengaidadingdan.log', print_r($order_num_update,true).PHP_EOL);
                        if (!$order_num_update) {
                            Db::rollback();
                        }
                        $listCoursePrice = self::getListCoursePrice($order_data);
                        file_put_contents('qiufengreong.log', print_r($listCoursePrice,true).PHP_EOL);
                        if ($listCoursePrice != 1000) {
                            Db::rollback();
                        }
                        file_put_contents('修改是否成功.log', print_r($order_update,true).PHP_EOL);
                        if ($order_update) {
                            Db::commit();
                        }
                    } else {
    ...
    {
        $result['out_trade_no'] = 157778257155;//大订单号
        Db::startTrans();
        try {
            $where = [
                'status' => 9,
...
//生成随机字符串
    ...
    //计算机构授课所得钱 (添加机构分润后流水)
    public static function getListCoursePrice($order_data)
    {
        ...
        foreach ($order_data as $k => $v) { //1普通课程，2体验课程，3活动课程，4秒杀课程，5综合体
            if ($v['price'] > 0) {
                if ($v['cou_status'] == 1) {
            ...
            $account_info = Crud::setAdd('account_member', $account_data);
        }
        if ($account_info) {
            return 1000;
        }
