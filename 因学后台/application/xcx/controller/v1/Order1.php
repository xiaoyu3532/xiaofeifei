...
//            'o.status'=>['in',[2,5,8]], //1未支付，2已支付，3申请退款，4已退款，5课程开始，6课程结束，7课程失败，8免费
    ...
    //减库存
    public static function setDelnum($data)
    {
        if ($data['cou_status'] == 1) { //1普通课程，2体验课程，3活动课程，4秒杀课程
            //yx_course
            $table = 'course';
            ...
            //查年当库存
            $Course_num_incs = Crud::setDecs($table, $where, 'surplus_num', 1);
            if (!$Course_num_incs) {
    ...
    public static function payExperienceCourse($data)
    {
        if ($data['status'] == 2) {
            $where = [
                'is_del'=>1,
//                'status'=>['in',[2,8]],
                'status'=>['neq',['2,5,6,8']], //1未支付，2已支付，3申请退款，4已退款，5课程开始，6课程结束，7课程失败，8免费，9支付中，10拒绝退款
                'uid'=>$data['user_id'],
                'cid'=>$data['cou_id'],
            ];
