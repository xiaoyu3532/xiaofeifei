...
//        }
    ...
    public static function withdrawStudentClass($student_member_id, $arrange_course_id)
    {
        ...
        foreach ($student_member_id as $k => $v) {
            ...
            $alias = 'zscl';
            $course_hour_record_data = Crud::getRelationData($table, $type = 2, $where_student_class_list, $join, $alias, $order = 'zscl.create_time desc', $field = 'zscl.course_enter_type,zscl.order_id,zscl.course_hour_record_id,zct.course_hour', 1, 10000000);
            //获取学生课时记录
            ...
            $where_course_hour_record = [
                'id' => $course_hour_record_data[0]['course_hour_record_id'],
            ];
            if ($course_hour_record['scheduled_give_num'] <= 0) {
                //减已排课时
                $scheduled_num = Crud::setDecs('zht_course_hour_record', $where_course_hour_record, 'scheduled_num', $course_hour);
                if (!$scheduled_num) {
                    Db::rollback();
                    throw new  UpdateMissException();
                }
                //加未排课时
                $stay_row_num = Crud::setIncs('zht_course_hour_record', $where_course_hour_record, 'stay_row_num', $course_hour);
                if (!$stay_row_num) {
                    Db::rollback();
                    throw new  UpdateMissException();
                }
                ...
                //减已排赠送课时
                $withdraw_give_num = $course_hour_record['scheduled_give_num'] - $course_hour;
                if ($withdraw_give_num <= 0) {
                    //如果赠送课少于撤回课时，1将已排赠送课改为0，2.加未排赠送课，3在减已排正式课，4加未排正式课
                    if ($withdraw_give_num == 0) {
                        //scheduled_give_num 减已排赠送课时
                        $scheduled_give_num = Crud::setDecs('zht_course_hour_record', $where_course_hour_record, 'scheduled_give_num', $course_hour_record['scheduled_give_num']);
                        if (!$scheduled_give_num) {
                            Db::rollback();
                            throw new  UpdateMissException();
                        }
                        //give_num 加赠送课时
                        $give_num = Crud::setIncs('zht_course_hour_record', $where_course_hour_record, 'give_num', $course_hour_record['scheduled_give_num']);
                        if (!$give_num) {
                            Db::rollback();
                            throw new  UpdateMissException();
                        }
                    } else {
                        //scheduled_give_num 将已排赠送课时改为0
                        $scheduled_give_num = Crud::setUpdate('zht_course_hour_record', $where_course_hour_record, ['scheduled_give_num' => 0]);
                        if (!$scheduled_give_num) {
                            Db::rollback();
                            throw new  UpdateMissException();
                        }
                        ...
                        //减已排课时
                        $scheduled_num = Crud::setDecs('zht_course_hour_record', $where_course_hour_record, 'scheduled_num', abs($withdraw_give_num));
                        if (!$scheduled_num) {
                            Db::rollback();
                            throw new  UpdateMissException();
                        }
                        //加未排课时
                        $stay_row_num = Crud::setIncs('zht_course_hour_record', $where_course_hour_record, 'stay_row_num', abs($withdraw_give_num));
                        if (!$stay_row_num) {
                            Db::rollback();
                            throw new  UpdateMissException();
                        }
            ...
            // 1此节课时间没有过，2时间已过（获取进度数
            if ($course_hour_record_data[0]['course_enter_type'] == 2) {

                $where_order = [
                    'order_id' => $course_hour_record_data[0]['order_id'],
                    'is_del' => 1,
                    'status' => ['NOTIN', [1, 6]] //1待付款，2待排课，3上课中，4已毕业，5已休学，6已退款 7已取消
                ];
                $order_data = Crud::getData('zht_order', 1, $where_order, 'course_num');
                if ($order_data['course_num'] <= $course_hour) {
                    //修改学生订单状态  yx_zht_order
                    $zht_order = Crud::setUpdate('zht_order', ['order_id' => $course_hour_record_data[0]['order_id']], ['status' => 2]);
                    if (!$zht_order) {
                        Db::rollback();
                        throw new UpdateMissException();
                    }
        ...
        if ($student_class_list_data) {
            foreach ($student_class_list_data as $k => $v) {
                ...
                if ($data['student_course_type'] == 2 || $data['student_course_type'] == 4) { //1等待上课，2已上课，3，请假，4.缺课，5休学
                    ...
                    //减已排课
                    $scheduled_num = Crud::setDecs('zht_course_hour_record', ['id' => $v['course_hour_record_id']], 'scheduled_num', $course_timetable_data['course_hour']);
                    if (!$scheduled_num) {
                        Db::rollback();
                        throw new  AddMissException();
                    }
