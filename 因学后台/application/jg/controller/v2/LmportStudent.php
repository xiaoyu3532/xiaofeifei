...
{
    //学生列表 1公海池，2业务员列表,3在读学员
    public static function getLmportStudentList($student_id = '', $phone = '', $student_status = '', $student_name = '', $mem_id = '', $time = '', $return_visit_type = '', $del_type = 1, $student_member_id = '', $page = 1, $pageSize = 8, $type = 1)
    {
...
//                'lm.is_del' => 1,
        ...
        if (empty($mem_id)) {
            ...
            $where['lsm.mem_id'] = ['in', $mem_ids];
        }
        (isset($mem_id) && !empty($mem_id)) && $where['lsm.mem_id'] = $mem_id;
        (isset($student_id) && !empty($student_id)) && $where['lsm.student_id'] = $student_id;
        (isset($student_member_id) && !empty($student_member_id)) && $where['lsm.id'] = $student_member_id;
        (isset($student_status) && !empty($student_status)) && $where['lsm.student_status'] = $student_status; //1公海池，2业务员列表,3在读学员
        if (isset($student_status) && !empty($student_status) && $student_status != 1) {
            (isset($mem_data['admin_user_id']) && !empty($mem_data['admin_user_id'])) && $where['lsm.salesman_id'] = $mem_data['admin_user_id'];
        }
        ...
        (isset($phone) && !empty($phone)) && $where['lsm.phone'] = ['like', '%' . $phone . '%'];
        //时间筛选
        if (isset($time) && !empty($time)) {
            $start_time = $time[0] / 1000;
            $end_time = $time[1] / 1000 + 86399;
        ...
        $join = [
            ...
            ['yx_admin_user au', 'lsm.salesman_id = au.id', 'left'], //业务员信息
        ];
        $alias = 'lsm';
        $info = Crud::getRelationData($table, 2, $where, $join, $alias, 'lsm.create_time desc', 'lsm.*,m.cname,m.province mprovince,m.area marea,m.city mcity,m.address msaddress,au.real_member_name salesman_name', $page, $pageSize);
        foreach ($info as $k => $v) {
            ...
            $info[$k]['get_into_time_Exhibition'] = date('Y-m-d H:i:s', $v['get_into_time']);
            ...
            if ($v['is_audition'] == 1) {
                $info[$k]['is_audition_Exhibition'] = '试听';
            } elseif ($v['is_audition'] == 2) {
                $info[$k]['is_audition_Exhibition'] = '未试听';
            }
...
//            ['prop' => 'salesman_name', 'name' => '业务员', 'width' => '', 'state' => ''],
            ...
            ['prop' => 'get_into_time_Exhibition', 'name' => '入池时间', 'width' => '', 'state' => ''],
            ['prop' => 'create_time_Exhibition', 'name' => '添加时间', 'width' => '', 'state' => ''],
...
//                'm.status' => 1,
    ...
    public static function getOwnLmportStudentField()
    {
        $data = [
            ...
            ['prop' => 'is_audition_Exhibition', 'name' => '是否试听', 'width' => '', 'state' => ''],
            ['prop' => 'create_time_Exhibition', 'name' => '添加时间', 'width' => '', 'state' => ''],
            ['prop' => 'return_visit_content', 'name' => '回访信息', 'width' => '210', 'state' => ''],
        ];
        return jsonResponseSuccess($data);
...
//            'm.status' => 1,
    ...
    public static function addLmportStudent()
    {
        ...
        $data['salesman_id'] = $account_data['admin_user_id'];
    ...
    public static function moveOwnLmportStudent($student_member_id)
    {
        ...
        $updata = [
            ...
            'student_status' => 2, //1公海池，2潜在学员，3在读学员
        ];
        $lmport_student_data = Crud::setUpdate('lmport_student_member', $where, $updata);
        if ($lmport_student_data) {
            return jsonResponseSuccess($lmport_student_data);
...
//            $StudentInfo = self::getLmportStudentList('', '', '', '', '', '', 1, $data['student_member_id'], $page = 1, $pageSize = 8, 2);
            $StudentInfo = self::getLmportStudentList('', '', '', '', '', '', '', $del_type = 1, $data['student_member_id'], '', '', 2);
//            return $StudentInfo;
    ...
    public static function addReturnVisit()
    {
        $data = input();
        $mem_data = self::isuserData();
        $data['salesman_id'] = $mem_data['admin_user_id'];
    ...
    public static function getReturnVisit()
    {
        ...
        if ($account_data['type'] == 2 || $account_data['type'] == 7) {
            ...
            $where = [
                'rv.is_del' => 1,
                'rv.student_member_id' => $data['student_member_id'],
                'rv.mem_id' => $data['mem_id'],
            ];
            (isset($account_data['admin_user_id']) && !empty($account_data['admin_user_id'])) && $where['rv.salesman_id'] = $account_data['admin_user_id'];
            $join = [
                ['yx_admin_user au', 'rv.salesman_id = au.id', 'left'], //业务员信息
            ];
            $alias = 'rv';
            $info = Crud::getRelationData('return_visit', $type = 2, $where, $join, $alias, 'create_time desc', 'rv.type,rv.content,rv.create_time,au.real_member_name salesman_name', $data['page'], $data['pageSize']);
            if (!$info) {
                throw new NothingMissException();
...
//                $info[$k][$v] = isemptydata($v);
    ...
    public static function getReturnVisitField()
    {
        $data = [
            ['prop' => 'create_time_Exhibition', 'name' => '添加时间', 'width' => '', 'state' => ''],
            ['prop' => 'returnVisitTypeName', 'name' => '状态', 'width' => '', 'state' => ''],
            ['prop' => 'salesman_name', 'name' => '业务员名', 'width' => '320', 'state' => ''],
            ['prop' => 'content', 'name' => '内容', 'width' => '100', 'state' => ''],
        ];
        return jsonResponseSuccess($data);
    }
    ...
    //验证学生是否绑定此机构
    public static function isBindingMember($data)
    {
        $where = [
            'mem_id' => $data['mem_id'],
            'student_id' => $data['student_id'],
        ...
        $student_member_id = Crud::getData('lmport_student_member', 1, $where, 'id');
        return $student_member_id;
    }

    //验证学生当前状态
    public static function isStudentStatus($student_member_id)
    {
        $where = [
            'id' => $student_member_id,
            'is_del' => 1
        ];
        $student_status_data = Crud::getData('lmport_student_member', 1, $where, 'student_status');
        return $student_status_data;
    }

    //修改学生状态
    public static function updateStudentStatus($student_member_id)
    {
        $where = [
            'id' => $student_member_id,
            'is_del' => 1
        ];
        $student_status_data = Crud::setUpdate('lmport_student_member', $where, ['student_status' => 3]);
        return $student_status_data;
    }
