...
{
    //获取管理员列表
    public static function getAdminUserList($page = 1, $real_member_name = '', $staff_phone = '', $mem_id = '', $role_id = '', $pageSize = 8)
    {
        ...
        if ($account_data['type'] == 2 || $account_data['type'] == 7) { //1用户，2机构
            ...
            if (empty($mem_id)) {
                ...
                $where['aur.mem_id'] = ['in', $mem_ids];
            } else {
                $where['aur.mem_id'] = $mem_id;
            }


        }
        (isset($real_member_name) && !empty($real_member_name)) && $where['a.real_member_name'] = ['like', '%' . $real_member_name . '%'];//管理员名称
        (isset($staff_phone) && !empty($staff_phone)) && $where['a.staff_phone'] = ['like', '%' . $staff_phone . '%'];//管理员手机号
...
//            unset($data['mem_id']);
            ...
            if ($AdminUser) {
                //修改人员登录机构ID mem_id
                $aa = Crud::setUpdate('login_account', ['username' => $data['username']], ['mem_id' => $data['mem_id']]);
...
//            if ($admin_user != 1) {
            ...
            if (isset($data['role_id']) && !empty($data['role_id'])) {
                //将所有的角色删除
                $del_user_role = Crud::setUpdate('admin_user_role', ['admin_user_id' => $admin_user_id, 'mem_id' => $data['mem_id']], ['is_del' => 2]);
