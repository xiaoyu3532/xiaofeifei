...
//            'st.is_del'=>1,
        ...
        $alias = 'c';
        $info = Crud::getRelationData($table, $type = 2, $where, $join, $alias, $order = '', $field = 'm.cname,c.id,c.recom,c.present_price,c.c_num,ca.name caname,cl.name clname,cu.name cuname,c.course_status,c.type,c.mid mem_id', $page);
        if (!$info) {
...
//            'type' => 1,
        ...
        $table = request()->controller();
        $info = Crud::getData($table, $type = 1, $where, $field = 'id,img,present_price,c_num,classroom_id,classroom_type,teacher_id,teacher_type,curriculum_id,curriculum_cid,original_price,curriculum_csid,title,start_time,end_time,start_age,end_age,surplus_num,teacher_name,arrange_time');
        if (!$info) {
            throw new NothingMissException();
        } else {
...
//        dump($Curriculum_data);
    ...
    //课程复用(普通课程复用 体验、秒杀课)
    public static function multiplexingjgCourses($course_id){
