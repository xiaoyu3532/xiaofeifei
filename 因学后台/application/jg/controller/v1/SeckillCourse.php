...
{
    //获取秒杀课程列表
    public static function getjgSeckillCourse($page = '1', $name = '', $cg_id = '', $st_id = '', $type = '', $course_status = '', $mem_id = '')
    {
...
//            'st.is_del'=>1,
        ...
        $alias = 'c';
        $info = Crud::getRelationData($table, $type = 2, $where, $join, $alias, $order = '', $field = 'c.id,c.mid mem_id,c.recom,c.present_price,c.c_num,ca.name caname,cl.name clname,cu.name cuname,c.course_status,c.type,c.examine_type,m.cname', $page);
        if (!$info) {
    ...
    //获取秒杀课程详情
    public static function getjgSeckillCoursedetails($course_id)
    {
        $where = [
            'is_del' => 1,
//            'type' => 1,
    ...
    //添加秒杀课程
    public static function addjgSeckillCourse()
    {
        $mem_data = self::isuserData();
        $data = input();
        if ($mem_data['type'] == 2) {
...
//        }
    ...
    //修改秒杀课程
    public static function setjgSeckillCourse()
    {
        $data = input();
        if ($data['img']) {
            $data['img'] = handle_img_deposit($data['img']);
...
//        }
    ...
    //删除秒杀课程
    public static function deljgSeckillCourse($course_id)
    {
        $where = [
            'is_del' => 1,
            'id' => $course_id,
    ...
    //上下架秒杀操作
    public static function editjgSeckillCourseType($course_id, $type)
    {
        $where = [
            'id' => $course_id,
        ];
    ...
    //秒杀课程开始结束操作
    public static function editjgSeckillCourseStatus($course_id, $course_status)
    {
        $where = [
            'id' => $course_id,
        ];
    ...
    //修改单个日期返回时间
    public static function editjgSeckillCourseTime($course_id)
    {
        $where = [
            'id' => $course_id,
            'type' => 1,
    ...
    //返回单日的时间段状态列表
    public static function getjgSeckillCourseTimeSection()
    {
        $data = input();
        $where = [
            'id' => $data['course_id'],
    ...
    //修改单个日期（入库）
    public static function addjgSeckillCoursesingleTime()
    {
        $data = input();
    ...
    //重新审核提交
    public static function setjgExamineSeckillCourse($seckillcourse_id, $type)
    {
        $where = [
            'id' => $seckillcourse_id,
        ];
    ...
    //复制秒杀课程
    public static function copyjgSeckillCourse()
    {
        $data = input();
        $table = $table = request()->controller();
        $info = Crud::copyjgCourses($data, $table, 4);
        if ($info == 1000) {
            return jsonResponseSuccess($info);
        } else {
            return $info;
