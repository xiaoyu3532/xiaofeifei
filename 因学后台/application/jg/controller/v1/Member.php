...
{
    //获取机构材料(详情)
    public static function getMemberMaterial()
    {
        //获取机构ID
...
//                'm.is_del'=>1,
            ...
            if ($binding_member) {
                ...
                $info['binding_member'][] = ['cname' => $info['cname'], 'mem_id' => $info['mem_id']];
            }
            $logo = $info['logo'];
            if ($info) {
    ...
    //获取机构信息
    public static function getjgMemberinfo()
    {
        $mem_data = self::isuserData();
        if ($mem_data['type'] == 2) {
            $where = [
    ...
    //修改机构信息（详情与简介机构轮播图）
    public static function setjgMemberinfo()
    {
        $mem_data = self::isuserData();
        $data = input();
        if ($mem_data['type'] == 2) {
