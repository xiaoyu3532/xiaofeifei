...
{
    //导出订单
    public function exportpcOrder($page = '1', $name = '', $order_id = '', $sname = '', $status = '', $cou_status = '', $time = '', $cname = '')
    {
        $where = [
            'o.is_del' => 1
        ];
...
//            ['yx_course c', 'o.cid = c.id', 'left'],  //课程
    ...
    //导出签到登记记录
    public function exportpcSignRegist()
    {
        $data = input();
        $where = [];
        ...
        $table = 'order';
        $list = Crud::getData($table, $type = 2, $where, $field = '*', $order = '', 1, 100000000);

        $objPHPExcel->getActiveSheet()->setCellValue('A1', '用户名称');
        $objPHPExcel->getActiveSheet()->setCellValue('B1', '手机号');
        $objPHPExcel->getActiveSheet()->setCellValue('C1', '报名时间');
        // 设置个表格宽度
        $objPHPExcel->getActiveSheet()->getColumnDimension('A')->setWidth(10);
        $objPHPExcel->getActiveSheet()->getColumnDimension('B')->setWidth(20);
        $objPHPExcel->getActiveSheet()->getColumnDimension('C')->setWidth(20);
        ...
        foreach ($list as $k => $val) {
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('B' . $i, $val['phone']);
            $objPHPExcel->getActiveSheet()->setCellValue('C' . $i, date('Y-m-d H:i:s', $val['create_time']));
        }

        // 1.保存至本地Excel表格
        $objWriter->save("签到登记表.xls");
        // 2.接下来当然是下载这个表格了，在浏览器输出就好了
        header("Pragma: public");
        header("Expires: 0");
        header("Cache-Control:must-revalidate, post-check=0, pre-check=0");
        ...
        header("Content-Type:application/octet-stream");
        header("Content-Type:application/download");
        header("Content-Disposition:attachment;filename=签到登记表.xls");
        header("Content-Transfer-Encoding:binary");
        $objWriter->save('php://output');
    }
