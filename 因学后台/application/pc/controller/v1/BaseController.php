...
class BaseController extends Controller
{
    ...
    public static function isuserData()
    {
        ...
        $user_data = Crud::isUserToken($token, 6);
        if (!$user_data) {
            throw new ISUserMissException();
        } else {
            return $user_data;
    ...
    //组合
    public static function fieldArray($arrye)
    {
        $array_Field = [];
        foreach ($arrye as $k => $v) {
            if (isset($v[2])) {
                $field = ['prop' => $v[0], 'name' => $v[1], 'width' => $v[2], 'state' => ''];
            } else {
                $field = ['prop' => $v[0], 'name' => $v[1], 'width' => '', 'state' => ''];
            }
            $array_Field[] = $field;
        }
        return $array_Field;
