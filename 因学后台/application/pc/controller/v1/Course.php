...
{
    //获取课程列表
    public static function getpcCourse($mem_id, $page = '1', $name = '', $cg_id = '', $st_id = '', $type = '', $course_status = '')
    {
        $where = [
            'c.is_del' => 1,
            'c.mid' => $mem_id,
    ...
    //获取课程详情
    public static function getpcCoursedetails($course_id)
    {
        $where = [
            'is_del' => 1,
//            'type' => 1,
        ...
        $table = request()->controller();
        $info = Crud::getData($table, $type = 1, $where, $field = 'id,img,present_price,c_num,classroom_id,classroom_type,teacher_id,teacher_type,curriculum_id,curriculum_cid,original_price,curriculum_csid,title,start_time,end_time,start_age,end_age,arrange_time,teacher_name');
        if (!$info) {
            throw new NothingMissException();
        } else {
    ...
    //添加课程
    public static function addpcCourse()
    {
        $data = input();
...
//        }
    ...
    //获取修改课程数据(开始时间于结束时间)
    public static function getpcCourseTime($course_id)
    {
        $where = [
            'id' => $course_id,
            'type' => 1,
    ...
    //修改课程
    public static function setpcCourse()
    {
        $data = input();
        if ($data['img']) {
            $data['img'] = handle_img_deposit($data['img']);
...
//        $data['present_price'] = $data['present_price'] * $data['c_num'];
    ...
    //删除课程
    public static function delpcCourse($course_id)
    {
        $where = [
            'is_del' => 1,
            'id' => $course_id,
    ...
    //上下架操作
    public static function editpcCourseType($course_id, $type)
    {
        $where = [
            'id' => $course_id,
        ];
    ...
    //开始，未开始，结束设置
    public static function editpcCourseStatus($course_id, $course_status)
    {
        $where = [
            'id' => $course_id,
        ];
    ...
    //修改单个日期返回开始结束时间
    public static function editpcTime($course_id)
    {
        $where = [
            'id' => $course_id,
            'type' => 1,
    ...
    //返回单日的时间段状态列表
    public static function getpcTimeSection()
    {
        $data = input();
        $where = [
            'id' => $data['course_id'],
    ...
    //修改单个日期操作（入库）
    public static function addpcsingleTime()
    {
        $data = input();
