...
class SeckillCourse extends BaseController
{
    //获取秒杀课程列表
    public static function getpcSeckillCourse($mem_id, $page = '1', $name = '', $cg_id = '', $st_id = '', $type = '', $course_status = '')
    {
        $where = [
            'c.is_del' => 1,
            'c.mid' => $mem_id,
        ...
        $alias = 'c';
        $info = Crud::getRelationData($table, $type = 2, $where, $join, $alias, $order = 'c.create_time desc', $field = 'c.id,c.recom,c.present_price,c.c_num,ca.name caname,cl.name clname,cu.name cuname,c.course_status,c.type,c.examine_type', $page);
        if (!$info) {
    ...
    //获取秒杀课程详情
    public static function getpcSeckillCoursedetails($course_id)
    {
        $where = [
            'is_del' => 1,
//            'type' => 1,
        ...
        $table = request()->controller();
        $info = Crud::getData($table, $type = 1, $where, $field = 'id,img,present_price,c_num,classroom_id,classroom_type,teacher_id,teacher_type,curriculum_id,curriculum_cid,original_price,curriculum_csid,title,start_time,end_time,start_age,end_age,surplus_num,arrange_time,teacher_name');
        if (!$info) {
            throw new NothingMissException();
        } else {
    ...
    //添加秒杀课程
    public static function addpcSeckillCourse()
    {
        $data = input();
...
//        }
    ...
    //获取修改秒杀课程数据(开始时间于结束时间)
    public static function getpcSeckillCourseTime($course_id)
    {
        $where = [
            'id' => $course_id,
            'type' => 1,
    ...
    //修改秒杀课程
    public static function setpcSeckillCourse()
    {
        $data = input();
        if ($data['img']) {
            $data['img'] = handle_img_deposit($data['img']);
...
//        $data['present_price'] = $data['present_price'] * $data['c_num'];
    ...
    //删除秒杀课程
    public static function delpcSeckillCourse($course_id)
    {
        $where = [
            'is_del' => 1,
            'id' => $course_id,
    ...
    //上下架操作秒杀课程
    public static function editpcSeckillCourseType($course_id, $type)
    {
        $where = [
            'id' => $course_id,
        ];
    ...
    //开始，未开始，结束设置操作秒杀课程
    public static function editpcSeckillCourseStatus($course_id, $course_status)
    {
        $where = [
            'id' => $course_id,
        ];
    ...
    //修改单个日期返回开始结束时间
    public static function editpcSeckillTime($course_id)
    {
        $where = [
            'id' => $course_id,
            'type' => 1,
    ...
    //返回单日的时间段状态列表
    public static function getpcSeckillTimeSection()
    {
        $data = input();
        $where = [
            'id' => $data['course_id'],
    ...
    //修改单个日期操作（入库）
    public static function addpcSeckillsingleTime()
    {
        $data = input();
    ...
    //获取秒杀课程列表
    public static function getpcSeckillCourseAll( $page = '1', $name = '', $cg_id = '', $st_id = '', $type = '', $course_status = '')
    {
        $where = [
            'c.is_del' => 1,
        ];
        ...
        $alias = 'c';
        $info = Crud::getRelationData($table, $type = 2, $where, $join, $alias, $order = 'c.create_time desc', $field = 'c.id,c.recom,c.present_price,c.c_num,ca.name caname,cl.name clname,cu.name cuname,c.course_status,c.type,c.examine_type,m.cname,m.ismember,c.start_time,c.original_price', $page);
        if (!$info) {
            throw new NothingMissException();
        } else {
            $num = Crud::getCountSelNun($table, $where, $join, $alias, $field = 'c.id');
    ...
    //平台审核秒杀课
    public static function setpcExamineSeckillCourse($seckillcourse_id,$type){
        $where = [
            'id' => $seckillcourse_id,
        ];
        $data = [
