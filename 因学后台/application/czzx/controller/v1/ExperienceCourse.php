...
{
    //获取体验课程列表
    public static function getczzxExperienceCourse($page = '1', $name = '', $cg_id = '', $st_id = '', $type = '', $course_status = '')
    {
...
//            'st.is_del'=>1,
        ...
        $alias = 'c';
        $info = Crud::getRelationData($table, $type = 2, $where, $join, $alias, $order = '', $field = 'c.id,c.recom,c.present_price,c.c_num,ca.name caname,cl.name clname,cu.name cuname,c.course_status,c.type,m.cname', $page);
        if (!$info) {
    ...
    //获取体验课程详情
    public static function getczzxExperienceCoursedetails($course_id)
    {
        $where = [
            'is_del' => 1,
//            'type' => 1,
...
//            }
    ...
    //添加体验课程
    public static function addczzxExperienceCourse()
    {
        $mem_data = self::isuserData();
        $data = input();
        //验证课程库存状态
...
//            $info['end_time'] = $info['end_time'] * 1000;
    ...
    //修改体验课程
    public static function setczzxExperienceCourse()
    {
        $data = input();
        if ($data['img']) {
            $data['img'] = handle_img_deposit($data['img']);
...
//        }
    ...
    //删除体验课程
    public static function delczzxExperienceCourse($course_id)
    {
        $where = [
            'is_del' => 1,
            'id' => $course_id,
    ...
    //上下架体验操作
    public static function editczzxExperienceCourseType($course_id, $type)
    {
        $where = [
            'id' => $course_id,
        ];
    ...
    //体验课程开始结束操作
    public static function editczzxExperienceCourseStatus($course_id, $course_status)
    {
        $where = [
            'id' => $course_id,
        ];
    ...
    //修改单个日期返回时间
    public static function editczzxExperienceCourseTime($course_id)
    {
        $where = [
            'id' => $course_id,
            'type' => 1,
    ...
    //返回单日的时间段状态列表
    public static function getczzxExperienceCourseTimeSection()
    {
        $data = input();
        $where = [
            'id' => $data['course_id'],
    ...
    //修改单个日期（入库）
    public static function addczzxExperienceCoursesingleTime()
    {
        $data = input();
    ...
    public static function copyjgExperienceCourses()
    {
        ...
        $info = Crud::copyCourses($data, $table, 2);
        if ($info == 1000) {
            return jsonResponseSuccess($info);
        } else {
            return $info;
