...
{
    //获取秒杀课程列表
    public static function getczzxSeckillCourse($page = '1', $name = '', $cg_id = '', $st_id = '', $type = '', $course_status = '')
    {
...
//            'st.is_del'=>1,
        ...
        $join = [
            ...
            ['yx_classroom cl', 'c.classroom_id = cl.id', 'left'], //教室
            ['yx_member m', 'c.mid = m.uid', 'left'], //机构
        ];
        $alias = 'c';
        $info = Crud::getRelationData($table, $type = 2, $where, $join, $alias, $order = '', $field = 'c.id,c.recom,c.present_price,c.c_num,ca.name caname,cl.name clname,cu.name cuname,c.course_status,c.type,c.examine_type,m.cname', $page);
        if (!$info) {
    ...
    //获取秒杀课程详情
    public static function getczzxSeckillCoursedetails($course_id)
    {
        $where = [
            'is_del' => 1,
//            'type' => 1,
    ...
    //添加秒杀课程
    public static function addczzxSeckillCourse()
    {
        $mem_data = self::isuserData();
        $data = input();
        if ($mem_data['type'] == 4) {
...
//        }
    ...
    public static function getczzxCourseTime($course_id)
    {
        $where = [
            'id' => $course_id,
            'type' => 1,
    ...
    //修改秒杀课程
    public static function setczzxSeckillCourse()
    {
        $data = input();
        if ($data['img']) {
            $data['img'] = handle_img_deposit($data['img']);
...
//        }
    ...
    //删除秒杀课程
    public static function delczzxSeckillCourse($course_id)
    {
        $where = [
            'is_del' => 1,
            'id' => $course_id,
    ...
    //上下架秒杀操作
    public static function editczzxSeckillCourseType($course_id, $type)
    {
        $where = [
            'id' => $course_id,
        ];
    ...
    //秒杀课程开始结束操作
    public static function editczzxSeckillCourseStatus($course_id, $course_status)
    {
        $where = [
            'id' => $course_id,
        ];
    ...
    //修改单个日期返回时间
    public static function editczzxSeckillCourseTime($course_id)
    {
        $where = [
            'id' => $course_id,
            'type' => 1,
    ...
    //返回单日的时间段状态列表
    public static function getczzxSeckillCourseTimeSection()
    {
        $data = input();
        $where = [
            'id' => $data['course_id'],
    ...
    //修改单个日期（入库）
    public static function addczzxSeckillCoursesingleTime()
    {
        $data = input();
    ...
    //重新审核提交
    public static function setczzxExamineSeckillCourse($seckillcourse_id,$type){
        $where = [
            'id' => $seckillcourse_id,
        ];
        $data = [
    ...
    //复制秒杀课程
    public static function copyczzxSeckillCourse()
    {
        $data = input();
        $table = $table = request()->controller();
        $info = Crud::copyCourses($data, $table,4);
        if ($info == 1000) {
            return jsonResponseSuccess($info);
        } else {
            return $info;
