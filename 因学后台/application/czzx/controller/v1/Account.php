...
{
    //获取成长中心构流水
    public static function getczzxAccount()
    {
        //验证密码
        $data = input();
    ...
    //展示成长中心支出条型图
    public static function getczzxDataimg($start_time = '', $end_time = '')
    {
        $mem_data = self::isuserData();
        if ($mem_data['type'] == 4) {
    ...
    //列表展示
    public static function getczzxAccountlist($page = 1)
    {
        $mem_data = self::isuserData();
        if ($mem_data['type'] == 4) {
            //获取机构名称
    ...
    //成长中心首页核心数据
    public static function getczzxIndexStatistics()
    {
    ...
    public static function getczzxIndexStatisticsImg($img_type = 1)
    {
        $mem_data = self::isuserData();
        if ($mem_data['type'] == 4) {
            $time = time();
    ...
    //成长中心首页财务明细
    public static function getczzxIndexFinance()
    {
...
//            $week = Time::getWeekMyActionAndEnd($time);
    ...
    public static function getczzxCumulative($account_type = 1)
    {
        $mem_data = self::isuserData();
        if ($mem_data['type'] == 4) {
...
//            $day_time = Time::getDay();
    ...
    //成长中心数据分析 （财务数据）
    public static function getczzxFinance()
    {
        $mem_data = self::isuserData();
        if ($mem_data['type'] == 4) {
            $time = time();
    ...
    //成长中心数据分析 （订单数据）
    public static function getczzxAnalysisOrder()
    {
        $mem_data = self::isuserData();
        if ($mem_data['type'] == 4) {
            $year_time = time();
    ...
    //成长中心订单分析 （订单笔数）
    public static function getczzxOrdernum()
    {
        $mem_data = self::isuserData();
        if ($mem_data['type'] == 4) {
            $time = time();
    ...
    //成长中心订单分析 (课程数据)
    public static function getczzxAnalysisCategory()
    {
        $mem_data = self::isuserData();
        if ($mem_data['type'] == 4) {
            $where = [
    ...
    //成长中心订单分析 (订单分析)
    public static function getczzxOrderAnalysis()
    {
        //获取本周开始时间及每天的时间
    ...
    //成长中心订单分析 (订单分析)
    public static function getczzxOrderNumAnalysis()
    {
        //获取本周开始时间及每天的时间
        $time_data = Time::getTimeWeekDay();
        $mem_data = self::isuserData();
    ...
    //成长中心订单分析（课程订单，订单分类分析）
    public static function getczzxOrderCategory()
    {
        $mem_data = self::isuserData();
        if ($mem_data['type'] == 4) {
            $where = [
...
//                'c.is_del' => 1,
    ...
    //成长中心订单分析（年龄分析）
    public static function getczzxOrderAge()
    {
        $mem_data = self::isuserData();
        if ($mem_data['type'] == 4) {
            $where = [
...
//                dump($v['age']);
            ...
            //排序
            $list_data = array_sort($list_data, 'value', SORT_DESC);
